...
**What it is:** Fargate is AWS’s serverless compute engine for containers, used by both Amazon ECS and EKS. It abstracts away all infrastructure management—you don’t provision, configure, scale, or patch EC2 instances. Instead, you only define your container requirements (vCPU, memory, networking, IAM), and Fargate automatically provides the compute environment. Each task runs in its own lightweight, isolated, dedicated runtime, removing the need to operate servers or manage cluster capacity.

**Mechanism:** When you launch a task, Fargate provisions a secure, isolated compute environment for it, ensuring dedicated CPU and memory without sharing a host with other workloads. You specify how much CPU and memory your task needs, and Fargate automatically handles resource provisioning, scaling, and infrastructure lifecycle. No cluster sizing, no AMIs, no patching, no EC2 Auto Scaling Groups—AWS fully manages the infrastructure layer while you focus solely on the container and task definition.
